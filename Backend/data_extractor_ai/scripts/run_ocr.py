...
def process_documents_to_label_studio(input_folder, output_dir, languages=['en']):
        ...
        try:
            ...
            if ext in pdf_exts:
                ...
                for page_num in range(len(doc)):
                    ...
                    img_name = (
                        f"{file_path.stem}_page_{page_num + 1}.png"
                    )
            ...
            # ---------------------------------------------------------
                ...
                with Image.open(file_path) as img:
                    ...
                    while True:
                        ...
                        img_name = (
                            f"{file_path.stem}_frame_{frame_num + 1}.png"
                        )

                        img_save_path = images_dir / img_name

                        rgb_img = img.convert("RGB")
                        rgb_img.save(img_save_path, "PNG")

                        extracted_pages.append(
                            (img_save_path, frame_num + 1)
                        )
            ...
            tqdm.write(
                f"[ERROR] Failed to process {file_path.name}: {e}"
            )
            continue

        # =============================================================
        # EACH PAGE = ONE LABEL STUDIO TASK
        # =============================================================

        for img_path, page_number in extracted_pages:
            ...
            # ---------------------------------------------------------
            # ONE TASK FOR THIS IMAGE
            # ---------------------------------------------------------

            page_url = (
                f"/data/local-files/?d="
                f"intermediate/extracted_images/{img_path.name}"
            )
    ...
    print(
        f"\n[SUCCESS] Generated {len(tasks)} page-level tasks!"
    )

    print(
        f"[SUCCESS] Saved tasks.json to: {output_json_path}"
    )
